...
def animate_ee(Y_test, Y_preds, labels, fps=100):
    fig, ax = plt.subplots()
    ax.set(xlim=[-1.25,1.25], ylim=[-0.25,1.25])

    # 기준 선 (reference)
    line_ref, = ax.plot([], [], 'k-', label='Ref')

    # 예측 선들, labels 리스트에서 이름을 가져옴
    lines_pred = [ax.plot([], [], '-', label=lab)[0] for lab in labels]

    def init():
        line_ref.set_data([], [])
        for ln in lines_pred:
            ln.set_data([], [])
        return [line_ref] + lines_pred

    def update(i):
        # Ref 궤적
        xs, ys = Y_test[:i+1, 0], Y_test[:i+1, 1]
        line_ref.set_data(xs, ys)

        # Pred 궤적
        for ln, Yp in zip(lines_pred, Y_preds):
            xp, yp = Yp[:i+1, 0], Yp[:i+1, 1]
            ln.set_data(xp, yp)

        return [line_ref] + lines_pred

    ani = FuncAnimation(
        fig, update, frames=len(Y_test),
...
def plot_state_rates(T, Z_ref, Z_pred=None, Z_mpc=None, filename=None):
    """
    Plot angular rates  α̇, β̇  over time.
    Parameters
    ----------
    T        : (N,) array_like
        Time stamps [s].
    Z_ref    : (N, 4) array_like
        Reference trajectory columns: 0=α, 1=β, 2=α̇, 3=β̇
    Z_pred   : (N, 4) array_like, optional
        Model-predicted trajectory (same column order).
    Z_mpc    : (N, 4) array_like, optional
        MPC-executed trajectory.
    filename : str, optional
        If given, the figure is saved via save_fig(fig, filename).
    """
    linewidth = 2
    fig = new_fig()                       # <- 기존 헬퍼 사용
    ax = fig.add_subplot(1, 1, 1)
    ax.set(xlabel='Time $t$ (s)',
           ylabel=r'Angular rate $\dot{\alpha}/\dot{\beta}$ (rad/s)')
    ax.set(xlim=[np.min(T), np.max(T)])

    colors = ['tab:blue', 'tab:orange', 'tab:red', 'tab:green']
    for i, rate in enumerate([r'\dot{\alpha}', r'\dot{\beta}']):
        # reference
        ax.plot(T, Z_ref[:, i + 2],
                linewidth=linewidth,
                label=rf'${rate}^{{\mathrm{{ref}}}}$',
                c=colors[i * 2])
        # prediction
        if Z_pred is not None:
            ax.plot(T, Z_pred[:, i + 2],
                    linestyle='--', linewidth=linewidth,
                    label=rf'$\widehat{{{rate}}}$',
                    c=colors[i * 2 + 1])
        # MPC
        if Z_mpc is not None:
            ax.plot(T, Z_mpc[:, i + 2],
                    linestyle='--', linewidth=linewidth,
                    label=rf'${rate}^{{\mathrm{{MPC}}}}$',
                    c=colors[i * 2 + 1])
